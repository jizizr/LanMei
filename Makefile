...
hitokoto:
	...
	@cd server/rpc_gen && go mod init ${ROOT_MOD}/server/rpc_gen && cwgo client --service rpc --type RPC --module ${ROOT_MOD}/server/rpc_gen --I ../../idl/ --idl ../../idl/rpc.thrift && rm go.mod go.sum
.PHONY: history
history:
	$(eval NAME := history)
	@cd ${APP_PATH} && mkdir -p ${NAME}
